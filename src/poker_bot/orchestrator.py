...
from poker_bot.types import ActionValidationError, GameEvent, GamePhase
...
class GameOrchestrator:
    ...
    async def run(self, max_hands: int | None = None, close_agents: bool = True) -> None:
        ...
        try:
            while not self._stop_requested:
                ...
                await self._flush_terminal_updates()
                hands_played += 1
    ...
    async def _run_current_hand(self) -> None:
        while not self.engine.is_hand_complete():
            ...
            recent_events = self._unseen_events_for(acting_seat)
            pending_error = self._pending[acting_seat].validation_error
            logger.debug(
                "Requesting action acting_seat=%s phase=%s recent_events=%s pending_error=%s",
                acting_seat,
    ...
    async def _flush_terminal_updates(self) -> None:
        for seat_id, agent in self.player_agents.items():
            unseen_events = self._unseen_events_for(seat_id)
            if not unseen_events:
                continue
            logger.debug("Flushing terminal updates seat_id=%s events=%s", seat_id, unseen_events)
            await agent.notify_terminal(unseen_events, self.engine.get_player_view(seat_id))
